...
namespace MobileHolographicRemoting
{
    ...
    public static class PoseUtils
    {
        ...
        /// <returns></returns>
        public static byte[] ToBytes(Vector3 position, Vector3 eulerRotation)
        {
            ...
            Buffer.BlockCopy(BitConverter.GetBytes(eulerRotation.x), 0, bytes, 3 * sizeof(float), sizeof(float));
            Buffer.BlockCopy(BitConverter.GetBytes(eulerRotation.y), 0, bytes, 4 * sizeof(float), sizeof(float));
            Buffer.BlockCopy(BitConverter.GetBytes(eulerRotation.z), 0, bytes, 5 * sizeof(float), sizeof(float));
